...
from sklearn.base import RegressorMixin, ClassifierMixin, MetaEstimatorMixin, BaseEstimator
...
class _GrowingTreeEstimator(MetaEstimatorMixin, object):
    ...
    def fit(self, X, y, **fit_params):
        ...
        if not self._warm_start:
            print("resetting estimator to null-state")
            self.est = self._treeEstimatorClass(n_estimators = self._ntrees_start, **self._est_params)

        print('call from %s' % (str(self)))
        ...
        print("round n_est meter")
        print("%d %d %f" % (0, n_est, meter))
        ...
        while niter < self._nrounds_stop and self._ntress_increment > 0:
            ...
            if meter - meter_prev_max <= self._min_score_improvement :
                ...
                if niter >= niter_best + self._nrounds_noimp :
                    print('no improvement for %d rounds' % (self._nrounds_noimp))
                    break
                continue
        ...
        if niter == self._nrounds_stop:
            print('Stopped after %d iterations' % (self._nrounds_stop))
        ...
        print("With %d estimators:: TrainScore(cv-test): %f , TrainScore(all): %f" % (n_est_best, meter_prev_max, self._score_func(self.est, X, y)))
        return self.est
    def predict(self, X):
...
def scoring_to_score_func(scoring, default):
    ...
    if scoring=='binary_roc_auc_score':
        ...
        return make_binary_scorer(roc_auc_score, greater_is_better=True, needs_proba=True)
    if scoring == 'binary_log_loss':
        from sklearn.metrics import log_loss
        return make_binary_scorer(log_loss, greater_is_better=False, needs_proba=True)
     
    if scoring=='accuracy_score':
        from sklearn.metrics import accuracy_score
        return make_scorer(accuracy_score, greater_is_better=True, needs_proba=False)
    if scoring=='precision_score':
        from sklearn.metrics import precision_score
        return make_scorer(precision_score, greater_is_better=True, needs_proba=False)
    if scoring=='f1_score':
        from sklearn.metrics import f1_score
        return make_scorer(f1_score, greater_is_better=True, needs_proba=False)
    ...
    if scoring == 'mean_squared_error':
        ...
        return make_scorer(mean_squared_error, greater_is_better=False, needs_proba=False)
    if scoring == 'r2_score':
        from sklearn.metrics import r2_score
        return make_scorer(r2_score, greater_is_better=False, needs_proba=False)
