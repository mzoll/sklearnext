...
class _GrowingTreeEstimator(MetaEstimatorMixin, object):
    ...
    def fit(self, X, y, **fit_params):
        ...
        logger.info("With %d estimators:: TrainScore(cv-test): %f , TrainScore(all): %f" % (
        n_est_best, meter_prev_max, self._score_func(self.est, X, y)))
        return self.est
...
def scoring_to_score_func(scoring, default):
    """ lookup the right scoring function even with a fallback """
    if callable(scoring):
        return scoring
    if scoring is None:
        if callable(default):
